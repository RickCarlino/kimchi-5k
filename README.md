...
- `bun run utils/spot-check.ts` — Quick sampling or batch clearing by `defRequestId`.
  - Sample: `bun run utils/spot-check.ts [--id UUID[,UUID2]]` prints up to 3 `term: def` lines per request.
  - Delete: `bun run utils/spot-check.ts --delete UUID[,UUID2]` sets `def` to `null` and clears `defRequestId` for those batches (so they can be regenerated).
- `bun run utils/patrol.ts` — LLM quality patrol over definitions that lack `llmCheckOn`. Batches of 50 audit entries, stamp `llmCheckOn`, store serious `concerns` (definition only), auto-fix incorrect POS to one of: `ADJ, ADV, CONJ, NOUN, VERB, AFFIX, DET, NUM, PRON, PRT, PUNCT, X`. Progress logs show batch counts, timing, throughput, ETA, and rolling averages. Edit the `USER_PROMPT` at the top of `utils/patrol.ts` to tune the checker.
- `bun run utils/apply-corrections.ts` — For entries with `concerns`, batches of 5 let GPT-5.1 either replace the definition, keep it, or null it out (for non-words). After applying, it clears concerns and stamps `llmCheckOn`. Uses structured JSON output for safety.
- `bun run utils/add-eng.ts` — Adds `eng` translations for entries lacking one. Batches of 100 send `"def (term)"` to GPT-5.1 and write back into `data/2-definitions.json` incrementally.
- `bun run utils/server.ts` — Tiny Bun webserver to browse `data/2-definitions.json` locally (in-memory). Filters by search, POS, has/missing definition, and concerns; shows metadata (`defRequestId`, `llmCheckOn`). Default port 3000; override with `PORT=4000 bun run utils/server.ts`.
- `bun run utils/stats.ts` — Prints JSON stats: total, missing definitions, needs corrections, OK, unchecked (`llmCheckOn` missing), with percentages.
- `bun run utils/def-lemma-frequency.ts` — Lemmatize every definition via Google NLP and write a frequency list of lemmas used in definitions to `data/definition-lemmas.json`. Sends up to 100 definitions per NLP call to balance throughput and timeouts; respects Google rate limits and runs in-memory.
- `bun run utils/def-lemma-filter.ts` — Filter the lemma frequency list: remove lemmas that already appear in `0-terms.json` or occur fewer than 2 times. Writes `data/definition-lemmas-filtered.json`.
